...
def detect_pose(video_path, output_path):
  ...
  def is_arm_up(landmarks):
    ...
    right_arp_up = right_elbow.y < right_eye.y

    return left_arm_up and right_arp_up
  ...
  for _ in tqdm(range(total_frames), desc='Processando vídeo'):
    ...
    if results.pose_landmarks:
      ...
      if is_arm_up(results.pose_landmarks.landmark):
          if not arm_up:
            arm_up = True
            arm_movements_counts += 1
          else:
            arm_up = False

          cv2.putText(frame, f'Movimento dos bracos: {arm_movements_count}',
                      (10, 30), cv2.FONT_HERSHEY_COMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
